...
def get_hand_ranking(hand):
    ...
    counts = {k: v for k, v in counts.items() if k != 'J'}
    cnt_vals = list(counts.values())
    joker_count = hand.count('J')
    if counts == {}:
        return 7
    maxi_count = max(counts.values())
    total_cnt = joker_count + maxi_count
    if joker_count == 0:
    ...
    if total_cnt == 3 and joker_count == 2:
        return 4
    elif total_cnt == 3 and joker_count == 1:
        cnt_vals.remove(2)
        if 2 in cnt_vals:
            return 5
        return 4
